...
public abstract class Producteur1Acteur extends Producteur1Valeurs implements IActeur {
	...
	private void produireFeve() {
		...
		if (this.getStocks().get(Feve.FEVE_MOYENNE_EQUITABLE).getQuantite()+this.production_mqe()<=75000000) {
			this.getStocks().get(Feve.FEVE_MOYENNE_EQUITABLE).addQuantite(this.production_mqe());
		} else {
			this.getStocks().get(Feve.FEVE_MOYENNE_EQUITABLE).setQuantite(a, 75000000);
		}
		if (this.getStocks().get(Feve.FEVE_MOYENNE).getQuantite()+this.production_mq()<=150000000) {
			this.getStocks().get(Feve.FEVE_MOYENNE).addQuantite(this.production_mq());
		} else {
			this.getStocks().get(Feve.FEVE_MOYENNE).setQuantite(a, 150000000);	
		}
		if (this.getStocks().get(Feve.FEVE_BASSE).getQuantite()+this.production_bq() <= 150000000){
			this.getStocks().get(Feve.FEVE_BASSE).addQuantite(this.production_bq());
		} else {
			this.getStocks().get(Feve.FEVE_BASSE).setQuantite(a, 150000000);
		}
