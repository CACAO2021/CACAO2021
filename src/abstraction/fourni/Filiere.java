...
public class Filiere {
	...
	public void initialiser() {
		...
		List<IFabricantChocolatDeMarque> fabricants = this.getFabricantsChocolatDeMarque();
		for (IFabricantChocolatDeMarque f : fabricants) {
			...
			for (ChocolatDeMarque c : produits) {
				if (!getProprietaireMarque(c.getMarque()).equals(f) && !(getProprietaireMarque(c.getMarque()) instanceof IDistributeurChocolatDeMarque)) {
					throw new IllegalStateException("L'acteur "+((IActeur)f).getNom()+" produit du chocolat dont il n'est pas le propriete de la marque et qui n'est pas une marque distributeur :"+c);
				}
	...
	/**
	 * @return La liste des marques de chocolat
	 */
	...
	public void next() {
		...
		for (ChocolatDeMarque choco : getChocolatsProduits()) {
			for (IDistributeurChocolatDeMarque distri : getDistributeurs()) {
				quantiteEnTG.put(choco.getMarque(), quantiteEnTG.get(choco.getMarque())+distri.quantiteEnVenteTG(choco));
				quantiteTotaleEnTG+=distri.quantiteEnVenteTG(choco);
			}
		}
		for (String marque : getMarquesChocolat()) {
			if (quantiteEnTG.get(marque)>(0.049*quantiteTotaleEnTG)) {
	...
	public List<IFabricantChocolatDeMarque> getFabricantsChocolatDeMarque() {
		List<IFabricantChocolatDeMarque> res = new ArrayList<IFabricantChocolatDeMarque>();
		for (IActeur a : this.acteurs) {
			if ((a instanceof IFabricantChocolatDeMarque) && (!this.laBanque.aFaitFaillite(a))) {
				res.add((IFabricantChocolatDeMarque)a);
			}
		}
		return res;
	}
	...
	public double prixMoyen(ChocolatDeMarque choco, int etape) {
		for (ClientFinal client : this.clientsFinaux) {
			if (client.getCategorie().equals(choco.getCategorie())) {
				return client.prixMoyen(choco, etape);
			}
		}
		return 0.0;
	}
	public double getVentes(ChocolatDeMarque choco, int etape) {
		for (ClientFinal client : this.clientsFinaux) {
			if (client.getCategorie().equals(choco.getCategorie())) {
				return client.getVentes(etape, choco);
			}
		}
		return 0.0;
	}
